...
async def format_sse(event_data: dict) -> str:
...
async def stream_plan(graph, query: str, request_obj=None) -> AsyncGenerator[str, None]:
    ...
    try:
        ...
        yield await format_sse({"event": "agent_started", "agent": "orchestrator"})
        ...
        async for event in graph.astream(state, stream_mode="updates"):
            ...
            for node_name, node_state in event.items():
                ...
                if "error" in node_state and node_state["error"]:
                    yield await format_sse({"event": "error", "error": node_state["error"]})
                    return
                
                # We can broadcast the completed agent
                yield await format_sse({"event": "agent_completed", "agent": node_name})
        ...
        yield await format_sse({"event": "error", "error": str(e)})
